...
- Upload “Hospital_Case_2” by using the “open file” button in the “processing” tab of WEKA. -In the “classify” tab press the “choose” button and select the “JRip” from the “rules” folder.
...
- Select the “percentage spilt” check box and set it on the 66% train set.
...
- At the end, select the produced rules and copy and paste them into a notepad file and save it in .txt format for the next step.
- Follow all the above steps for “Hospital_Case_3_2” (the combination of case 2 and case 3).

**Note:** we used three kinds of rule-based classifiers (OneR, Part, and JRipt) in our study. Other classifiers can be evaluated with the above-explained steps (related config files are exist in Hospital/Machine Learning config). According to the better results of “JRip” in terms of accuracy and rules number, we chose this classifier and continued the rest with it.

### Rule mapping

- Rule_Mapper.m is a MATLAB code that was developed in MATLAB (v 2018b) in order to map JRip produced rules in WEKA to rule defining language of the siddhi CEP engine.
...
- Finally, the rules in the "WekaExtractedRules.txt" file are changed to siddhi rule defining language (the new rules with new syntax are in "Rule_Map.txt" that will be created by running the code.) and can be used directly into the engine rule set.
...
- For the rules we follow two approaches:
    - First:
        - Add the rules which were excluded from the “Hospital_Case_2” file to the engine.
    ...
    - Second:
        - Add the rules which were excluded from the “Hospital_Case_3_2” (the combination of case 2 and case 3) file to the engine (It means that the case 3 data was added to case 2 and the rules were updated based on the new subset (the rule adaption manner)).
        - Add the “Hospital_Case_4” (manually by "Siddhi event simulation") file as input and compare the output with the “Hospital_Case_4” class for computing the accuracy.
        - Compare the result with the first approach.

**Note:** for personalization in the data preparation part select just one of the cases (as you wish) and divide it into three segments. Then, follow the above parts and steps, exactly.
...
- Three approaches to feature selection were evaluated in this study.
	...
	- Combination method
		...
		- From each batch attributes with at least three repetitions in each batch were added to the new subset.
...
- Select the “percentage spilt” check box and set it on 66% train set.
- In the below list choose “class”.
- Finally, press the “start” button and wait until the learning gets completed.
- In the classifier output box, you can see the produced rules and the results of the “Jrip” classifier.
- At the end, select the produced rules and copy and paste them into a notepad file and save it in .txt format for the next step.
...
- For the rules we follow two approaches:
    - First:
	    - Add the rules which were excluded from the “mHealth_Subject_6_Proposed” file to the engine.
	...
	- Second
	    - Add the rules which were excluded from the “mHealth_subject_6_10_Proposed” (the combination of subject 6 and case 10) file to the engine (It means that the subject 10 data was added to subject 6 and the rules were updated based on a new subset (the rule adaption manner)).
...
**Note:** for personalization in the data preparation part, select just one of the cases (as you wish) and divided it into three segments. Then, follow the above parts and steps, exactly.
